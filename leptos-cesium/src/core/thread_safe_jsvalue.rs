//! Thread-aware wrapper for values—primarily Cesium `JsValue` handles—that need to cross Leptos
//! effects safely.
//!
//! This is adapted from the implementation in `leptos-leaflet` and is intentionally general so it
//! can wrap any type, but the primary use-case is enforcing that Cesium objects created on the
//! browser thread are only accessed on that same thread.
...
impl<T> Drop for ThreadSafeJsValue<T> {
    ...
    /// # Panics
    ///
    /// Panics if the value is not on the current thread, except when the value does not need to be
    /// dropped.
    #[track_caller]
    fn drop(&mut self) {
...
impl<T> ThreadSafeJsValue<T> {
    ...
    /// Returns whether the wrapped value can be accessed on this thread.
...
/// Safety: we assert that the thread_id is the same as the current thread_id when we dereference
/// the value.
...
/// Helper macro to implement `From<$type>` for `ThreadSafeJsValue<$type>` while adding an optional
/// type alias.
#[allow(dead_code)]
#[macro_export]
